...
//#2 not use
/*ACTION SimpleAssets::verifyalibi( name user , uint64_t nft_id, checksum256 nft_hash){
	require_auth(user)
	checksum256 tmp_hash;

	sassets assets_f( _self, nft_id);
	const auto itr = assets_f.find(nft_id);
	
	if( itr == assets_f.end()){
		check(false, "Not exist NFT !");
	}
	else{
		const auto iter nfthasht.find(nft_id);
			if( iter == nfthasht.end()){
				check(false, "Not exist NFT in alibicahin...");
			}
			else{
					tmp_hash = iter->nft_hash;
			}
	}

	if( tmp_hash == nft_hash ) { //success  }
	else { //failure }
}*/
...
ACTION SimpleAssets::authorreg( name author, string info, string fieldtypes, string priorityimg ) {

	require_auth("alibicontrac"_n);

	require_auth( author );
	require_recipient( author );

	check( info.size() > 3, "Data field is too short. Please tell us about yourselves." );
...
ACTION SimpleAssets::authorupdate( name author, string info, string fieldtypes, string priorityimg ) {
	require_auth( author );
	require_recipient( author );

	auto itr = authort.require_find( author.value, string("author " + author.to_string() + " not registered").c_str() );

	if ( info.empty() && fieldtypes.empty() ) {
...
( offer )( canceloffer )( claim ) ( setnfthash ) ( setcerthashd )
